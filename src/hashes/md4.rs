...
//!
//! This is copied from RustCrypto
//! Challenge 31 requires adapting this to create a valid hash via length extension
...
/// Added for challenge 31 - allows the creation of a hasher from a known final state
impl Md4Core {
    fn new_from_state(state: [u32; 4], block_len: u64) -> Md4Core {
        Md4Core { block_len, state }
    }
}

/// Convenience method since CoreWrapper is not in this crate
pub fn new_from_state(state: [u32; 4], len: u64) -> Md4 {
    // Slightly hacky but since len is measured in blocks not bytes, this is a simple conversion
    // for this use case where the block length will always be 64
    let len = len / 64;

    CoreWrapper::from_core(Md4Core::new_from_state(state, len))
}
