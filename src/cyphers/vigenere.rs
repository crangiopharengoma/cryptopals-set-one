...
pub fn decrypt(message: &[u8], key: &[u8]) -> Vec<u8> {
    // decrypting is just repeating the process of encrypting
    encrypt(message, key)
}

pub fn break_encryption(encrypted_message: &[u8]) -> Vec<u8> {
    ...
    let key_size = (2..max_key_length).min_by_key(|key_size| {
        string_heuristics::normalized_edit_distance(encrypted_message, key_size)
    }).unwrap();

    let transposed_chunks: Vec<Vec<u8>> = (0..key_size)
        .map(|i| {
            encrypted_message
                .chunks(key_size)
                .map(|chunk| {
    ...
    let key: Vec<u8> = transposed_chunks.iter()
        .map(|inner| {
            caesar_cypher::find_key(inner).expect("key brute force failed")
        })
        .collect();
